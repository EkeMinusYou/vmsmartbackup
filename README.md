...
> [!NOTE]
> daily ジョブは `latest/` を入力にコピーを作るため、先に hourly ジョブが実行されて `latest/` が作成されている必要があります。初回起動直後に daily が走るとコピー元が無く失敗するので、最初の hourly 実行（次の毎時 0 分）以降が有効なバックアップになります。
...
# 日次バックアップから復元する例
vmrestore \
  -src=gs://<your-backup-bucket>/daily/<YYYYMMDD>/vmstorage-0 \
