...
事前ビルド済みのマルチアーキ（`linux/amd64` / `linux/arm64`）イメージを GitHub Container Registry (ghcr.io) で配布しています。

```
ghcr.io/ekeminusyou/vmsmartbackup:<tag>
```

| タグ | 内容 |
| --- | --- |
| `vX.Y.Z` | リリースタグ（不変・本番推奨） |
| `latest` | `main` ブランチの最新ビルド（可変） |

リリースタグ（`v*`）を push するとイメージが自動ビルド・公開されます（`.github/workflows/publish.yml`）。本番では `latest` ではなく `vX.Y.Z` のような固定タグを指定してください。
...
- 配布イメージ（[ghcr.io](#コンテナイメージ)）を使う場合は追加のビルド環境は不要。自分でイメージをビルドする場合は Docker、ローカルでビルド・実行する場合は Go 1.26 以上。
...
配布イメージ `ghcr.io/ekeminusyou/vmsmartbackup:<tag>` をそのまま使えます。プライベートレジストリへ持ち込みたい場合や独自に改変する場合のみ、自分でビルド・push します。
...
VictoriaMetrics を Helm チャートで管理している場合は、values の `vmstorage.extraContainers` に以下を追記します（`<version>` / `<your-backup-bucket>` は環境に合わせて置き換え）。

```yaml
vmstorage:
  extraContainers:
    - name: vmsmartbackup
      image: ghcr.io/ekeminusyou/vmsmartbackup:<version>
...
docker run --rm \
  ...
  -p 8000:8000 \
  ghcr.io/ekeminusyou/vmsmartbackup:<version>
```
